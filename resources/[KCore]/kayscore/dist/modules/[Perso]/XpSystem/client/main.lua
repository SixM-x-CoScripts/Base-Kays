...
RegisterCommand(kayscoreConfigServ.XpSystems.Command, function()
...
RegisterKeyMapping(kayscoreConfigServ.XpSystems.Command, 'Ouvrir la Barre d\'exp', 'keyboard', kayscoreConfigServ.XpSystems.Touche)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(kayscoreConfigServ.XpSystems.Wait)
...
Citizen.CreateThread(function()
    while (true) do
		...
		if XNL_CurrentPlayerXP <= 600 then
			...
			for i = 1, #kayscoreConfigServ.XpSystems.ToucheDesactiver, 1 do
				DisableControlAction(kayscoreConfigServ.XpSystems.ToucheDesactiver[i].group, kayscoreConfigServ.XpSystems.ToucheDesactiver[i].key, true)

				if IsDisabledControlJustPressed(kayscoreConfigServ.XpSystems.ToucheDesactiver[i].group, kayscoreConfigServ.XpSystems.ToucheDesactiver[i].key) then
					SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)

					if kayscoreConfigServ.XpSystems.ToucheDesactiver[i].message then
                        SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)
                        ESX.ShowNotification(kayscoreConfigServ.XpSystems.ToucheDesactiver[i].message)
					end
				end
			end
		end
