...
local tireBurstMaxNumber = VEHICLE_DAMAGE.randomTireBurstInterval * 1200; 										-- the tire burst lottery runs roughly 1200 times per minute
if VEHICLE_DAMAGE.randomTireBurstInterval ~= 0 then tireBurstLuckyNumber = math.random(tireBurstMaxNumber) end	-- If we hit this number again randomly, a tire will burst.
...
local function fscale(inputValue, originalMin, originalMax, newBegin, nslided, curve)
	...
	if (nslided > newBegin) then
		NewRange = nslided - newBegin
	else
	  NewRange = newBegin - nslided
	  invFlag = 1
	end
...
if VEHICLE_DAMAGE.torqueMultiplierEnabled or VEHICLE_DAMAGE.limpMode then
	Citizen.CreateThread(function()
		while true do
			Citizen.Wait(1)
			if VEHICLE_DAMAGE.torqueMultiplierEnabled or VEHICLE_DAMAGE.sundayDriver or VEHICLE_DAMAGE.limpMode then
				if pedInSameVehicleLast then
					local factor = 1.0
					if VEHICLE_DAMAGE.torqueMultiplierEnabled and healthEngineNew < 900 then
						factor = (healthEngineNew+200.0) / 1100
					end
					if VEHICLE_DAMAGE.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(VEHICLE_DAMAGE.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(VEHICLE_DAMAGE.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(VEHICLE_DAMAGE.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(VEHICLE_DAMAGE.sundayDriverBrakeCurve*2.0))
							end
						else
					...
					if VEHICLE_DAMAGE.limpMode == true and healthEngineNew < VEHICLE_DAMAGE.engineSafeGuard + 5 then
						factor = VEHICLE_DAMAGE.limpModeMultiplier
					end
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * VEHICLE_DAMAGE.damageFactorEngine * VEHICLE_DAMAGE.classDamageMultiplier[vehicleClass]

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * VEHICLE_DAMAGE.damageFactorBody * VEHICLE_DAMAGE.classDamageMultiplier[vehicleClass]

			healthPetrolTankCurrent = GetVehiclePetrolTankHealth(vehicle)
			if VEHICLE_DAMAGE.compatibilityMode and healthPetrolTankCurrent < 1 then
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * VEHICLE_DAMAGE.damageFactorPetrolTank * VEHICLE_DAMAGE.classDamageMultiplier[vehicleClass]

			if healthEngineCurrent > VEHICLE_DAMAGE.engineSafeGuard+1 then
				SetVehicleUndriveable(vehicle,false)
			end

			if healthEngineCurrent <= VEHICLE_DAMAGE.engineSafeGuard+1 and VEHICLE_DAMAGE.limpMode == false and IsVehicleDriveable(vehicle, false) then
				openUndriveableMenu(vehicle)
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > (healthEngineCurrent - VEHICLE_DAMAGE.engineSafeGuard) then
					...
					if healthEngineNew < VEHICLE_DAMAGE.engineSafeGuard then
						healthEngineNew = VEHICLE_DAMAGE.engineSafeGuard
					end

					-- Prevent Explosions
					if VEHICLE_DAMAGE.compatibilityMode == false and healthPetrolTankCurrent < 750 then
				...
				local newFDeformationDamageMult = fDeformationDamageMult ^ VEHICLE_DAMAGE.deformationExponent	-- Pull the handling file value closer to 1
				if VEHICLE_DAMAGE.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult', newFDeformationDamageMult * VEHICLE_DAMAGE.deformationMultiplier) end  -- Multiply by our factor
				if VEHICLE_DAMAGE.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', VEHICLE_DAMAGE.weaponsDamageMultiplier/VEHICLE_DAMAGE.damageFactorBody) end -- Set weaponsDamageMultiplier and compensate for damageFactorBody
				...
				local newFCollisionDamageMultiplier = fCollisionDamageMult ^ VEHICLE_DAMAGE.collisionDamageExponent	-- Pull the handling file value closer to 1
				...
				--Modify it by pulling all number a towards 1.0
				local newFEngineDamageMult = fEngineDamageMult ^ VEHICLE_DAMAGE.engineDamageExponent	-- Pull the handling file value closer to 1
			...
			lastVehicle=vehicle
			if VEHICLE_DAMAGE.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
				-- We just got out of the vehicle
				lastVehicle = GetVehiclePedIsIn(ped, true)
				if VEHICLE_DAMAGE.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult', fDeformationDamageMult) end -- Restore deformation multiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fBrakeForce', fBrakeForce)  -- Restore Brake Force multiplier
				if VEHICLE_DAMAGE.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', VEHICLE_DAMAGE.weaponsDamageMultiplier) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
