...
function StartTravailleJardinier()
	if not ESX.Game.IsSpawnPointClear(vector3(-1342.414185, 113.849487, 56.222809), 6.0) then
	local veh = ESX.Game.GetClosestVehicle(vector3(-1342.414185, 113.849487, 56.222809))
	RemoveObj(veh)
	end

	ESX.Game.SpawnVehicle(`mower`, vector3(-1342.414185, 113.849487, 56.222809), 59.14, function(veh)
		...
		while AuTravailleJardinier do
			...
			for k, v in pairs(workzone) do
				...
				if count == random and AuTravailleJardinier then
					...
					TriggerServerEvent("jobs_civil:jardinierSetPoint", v.pos)
					...
					while not EnAction and AuTravailleJardinier do
						...
						if dstToMarker <= 3.0 and AuTravailleJardinier then 
							if GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), false)) ~= 1783355638 then
