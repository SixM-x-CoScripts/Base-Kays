    ...
    $('#itemFast-1').droppable({
        drop: function(event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                ...
                $.post("http://inv/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot: 1
                }));
            }
...
$(function() {
    ...
    $('#button\\.raccourci-1').click(function() {
        $.post('http://inv/category', JSON.stringify({
            type: 'all'
        }));
    });
...
$(document).ready(function() {
    ...
    $('.middle-clothes-part-left .item-box').off().click(function(){
        $.post("http://inv/removeClothes",JSON.stringify({component:$(this).attr('id')}))
    })
    ...
    $('#useItem').droppable({
        ...
        drop: function(event, ui) {
            ...
            itemData = ui.draggable.data("item");
            if (itemData.usable) {
                $.post("http://inv/useItem", JSON.stringify({
                    item: itemData,
    ...
    $('#giveItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://inv/giveItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#renameItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://inv/renameItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#deleteItem').droppable({
        ...
        drop: function(event, ui) {
            itemData = ui.draggable.data("item");
            $.post("http://inv/deleteItem", JSON.stringify({
                item: itemData,
                // number: parseInt($("#count").val())
            }));
        }
    ...
    $('#left-inventory').droppable({
        drop: function(event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inv/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inv/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "normal" && itemInventory === "fast") {
                disableInventory(500);
                $.post("http://inv/TakeFromFast", JSON.stringify({
                    item: itemData
                }));
            } else if (type === "vault" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inv/TakeFromVault", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inv/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            }
    ...
    $('#right-inventory').droppable({
        drop: function(event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://inv/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://inv/PutIntoProperty", JSON.stringify({
                    item: itemData,
                }));
                ...
                disableInventory(500);
                $.post("http://inv/PutIntoVault", JSON.stringify({
                    item: itemData,
                    // number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://inv/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                }));
            }
        }
    ...
    document.getElementById("textArea").addEventListener("keyup", function(event) {
        ...
        $.post("http://inv/send", JSON.stringify({"text": document.getElementById("textArea").value.trim()}));
        document.getElementById("textArea").value = "";
        $("#keyboard-input").hide();
        }
    ...
    $('#button\\.showButton').click(function(){
        $.post("http://inv/giveCard", JSON.stringify({
            info: idcard,
            type: idcardType
        }));
        $(".menu").hide();
    ...
    $('#button\\.chooseButton').click(function(){
        ...
        $.post("http://inv/lookCard", JSON.stringify({
            info: idcard,
            type: idcardType
        }));
        $(".menu").hide();
