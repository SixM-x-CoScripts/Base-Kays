...
--- todo ajouter un timer de 15 mins pour aller au point et prevenir la police/SASPN toute les 5 mins la position du gofaster
...
Citizen.CreateThread(function()
    ...
    local ped = CreatePed(4, model, Config.GoFast.Pos, 46.130657196045, false, true)
...
showGoFastMenu = function()
    ...
    SetCamCoord(camera, 1628.203125, -2287.949951, 105.785881)
    ...
    while mainMenu do
        RageUI.IsVisible(mainMenu, function()
            if isAllowedToProgram then
                RageUI.Button("Réaliser un GoFast", "Attention a pas te faire choper", {RightLabel = ">>"}, true, {
                    onSelected = function()
						RageUI.CloseAll()
...
finishGoFast = function()
	...
	if plate == '60LYV000' then
		ESX.ShowAdvancedNotification("GoFast", "Livraison GoFast", "laisse le véhicule se garrer tout seul.", "CHAR_LESTER_DEATHWISH", 8)
...
-- Ouverture de toute les portes
		...
		ESX.ShowAdvancedNotification("Anonyme", "Livraison GoFast", "Calcule du butin en cours ...", "CHAR_LESTER_DEATHWISH", 8)
		FreezeEntityPosition(ped, true)
		Wait(6000)
		RemoveBlip(BlipsGoFast)
		local playerPed = PlayerPedId()
		secured_TriggerServerEvent("kayscore:CacaGofast", reward)
		...
		ESX.ShowAdvancedNotification(
			"Anonyme", 
			string.format("%sLivraison GoFast", kayscoreConfig.Color), 
			"Hein ? C'est quoi ça ? C'est pas la voiture du GoFast !", 
...
GoFastBlips = function()
	...
	SetBlipScale(BlipsGoFast, 0.5) -- set scale
