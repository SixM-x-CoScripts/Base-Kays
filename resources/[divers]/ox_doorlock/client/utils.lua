...
local function pickLock(entity)
	...
	if rand == 1 then
		...
		ESX.ShowNotification("Votre crochet s'est cassé")
		--lib.notify({ type = 'error', description = locale('lockpick_broke'), position = 'bottom' })
	end
...
RegisterNUICallback('notify', function(data, cb)
    cb(1)
    --lib.notify({title = data, position = 'bottom'})
